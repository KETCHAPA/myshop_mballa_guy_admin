...
(function () {
    var data = [];
    JSON.parse($('#data-target-js').text()).forEach(function (item) {
        data.push({
            'Photo': item.photo == null ? item.gender == 0 ? '../assets/images/backend/no_picture/avatar.png' : '../assets/images/backend/no_picture/avatarFemme.jpg' : '../assets/images/backend/admin/' + item.photo,
            'Nom': item.firstname,
            'Prenom': item.lastname,
            'Email': item.email,
            'Email': item.email,
            'Adresse': item.street == null ? '' : item.street,
            'Poste' : item.isAdmin == 1 ? 'Administrateur' : 'Super Administrateur',
            'Sexe': item.gender == 0 ? 'Homme' : 'Femme',
            'Status': item.isBlock == 0 ? "<i class=\"fa fa-circle font-success f-12\"></i>" : "<i class=\"fa fa-circle font-warning f-12\"></i>",
        })
    })
    var db = {
